...
print('-INFO- Ratio of food that has informed energy within -10% to +30%',
...
def print_extra_category_counts(df):
    category_dict = get_product_category_dict(df)
    for category in ['chicken', 'pork', 'beef', 'ham', 'kebab', 'fish', 'cheese', 'shrimp', 'porridge', 'hamburger', 'pasta', 'salmon', 'cake', 'lamb', 'tofu']: # TODO: for cakes could create 2 categories for salty and sweet
        food_count = 0
        categoty_count = 0
        for key, value in category_dict.items():
            food_count += value if (category in key.lower()) else 0
            categoty_count += 1 if (category in key.lower()) else 0
        print('Food categories {} and {} foods with {}'.format(categoty_count, food_count, category))
...
def save_distribution_of_ingredient_count_per_category(df):
    ...
    plt.savefig('assets/analytics_of_input_data/ingredient_count_per_category.png', bbox_inches='tight')
...
def get_product_category_dict(df):
    ...
    for category in df['category']:
        if not cat_dict.get(category):
            ...
            cat_dict[category] += 1
    category_dict = {k: v for k, v in sorted(cat_dict.items(), key=lambda item: item[1])}
